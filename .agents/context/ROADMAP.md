...
- [x] **Admin system Phase 1 (API)** — `AdminUser` model, admin JWT auth, dual storage backend selectable via `STORAGE_BACKEND=s3|postgres` (S3-compatible default — Backblaze B2 / R2 / MinIO / AWS S3; Postgres-blob alternative for self-hosted single-DB deploys with HTTP Range support over `bytea`), CRUD API for uploaded tracks, music source toggle. `TracksService` and `SearchService` merge results from enabled sources. _(ADR-0020; storage made provider-agnostic in ADR-0025; postgres-blob backend added in ADR-0026)_
...
- [-] **Direct upload source** — superseded by admin upload system above. Admin uploads tracks via `POST /api/admin/tracks` → S3-compatible storage (Backblaze B2 by default) or Postgres `bytea` blob (`STORAGE_BACKEND=postgres`, ADR-0026).
