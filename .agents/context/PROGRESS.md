...
- 2026-04-26 · _(this PR)_ · Follow artists (server + web + Mini App). New `Follow` Prisma model (composite PK `[userId, artistId]`, free-string `artistId` on the `Like`/`PlayHistory` precedent — see ADR-0016). New `FollowsModule` exposes `GET /api/me/follows` (hydrated via Jamendo, newest-first), `GET /api/me/follows/ids` (cheap "is following?" check), `POST /api/me/follows/:artistId` (idempotent upsert), `DELETE /api/me/follows/:artistId`. Web `/artists/[id]` gets a new `FollowButton` (optimistic, sign-in CTA for guests); Mini App gets a compact equivalent. Both library pages gain a "Following" section above the existing Liked/Recently played sections. 10 new FollowsService tests (63 total, all green). FEATURES/ROADMAP/ARCHITECTURE/DECISIONS updated; ADR-0016 added.
