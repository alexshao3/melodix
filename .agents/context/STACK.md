...
| `API_PUBLIC_URL`       | no (optional)                  | `storage/backends/postgres.backend.ts`     | Optional. If set, upload URLs become `${API_PUBLIC_URL}/api/storage/<key>` (use when fronting the API with a CDN). If empty, URLs are stored as relative `/api/storage/<key>` and resolved by the Next.js rewrite proxy in web/miniapp/admin (ADR-0029). |
| `API_INTERNAL_URL`     | no (web/miniapp/admin only)    | `apps/{web,miniapp,admin}/next.config.mjs` | Internal URL the Next servers use to proxy `/api/storage/*` to the API container. Defaults to `http://api:4000` (docker-compose service hostname). Override only if you renamed the service or run the Next apps outside Docker.                         |
| `AENEAS_URL`           | no                             | `admin-tracks/lyrics-aligner.service.ts`   | Internal URL of the Aeneas forced-aligner sidecar. Defaults to `http://aeneas:8000` (docker-compose service hostname). Used by the `POST /api/admin/tracks/:id/auto-sync-lyrics` endpoint. ADR-0030.                                                     |
