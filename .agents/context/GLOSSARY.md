...
| **LRC**                     | Standard time-synced lyrics format `[mm:ss.xx]line text`. Stored in `Track.syncedLyrics` (TEXT) and parsed by `packages/shared/src/lrc.ts`. The "x.xx" is centiseconds, not millis.        |
| **Synced lyrics**           | The LRC string on `Track.syncedLyrics`, generated by `POST /api/admin/tracks/:id/auto-sync-lyrics`. Player highlights the active line by binary-searching against `audio.currentTime`.     |
| **Forced alignment**        | The technique behind auto-sync: given an audio file _and the exact lyrics text_, compute timestamps deterministically (DTW on MFCC vs. an eSpeak TTS reference). No ASR, no hallucination. |
| **Aeneas**                  | The Python forced-aligner library (`aeneas==1.7.3.0`) backing our sidecar service. Lives in `services/aeneas-aligner/`. Uses eSpeak + ffmpeg under the hood. ADR-0030.                     |
| **Aeneas sidecar**          | The `services/aeneas-aligner/` Docker container. Exposes `POST /align` on the internal `melodix` Docker network only — not published to the host. The API calls it via `AENEAS_URL`.       |
