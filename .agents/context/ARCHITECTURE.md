...
| `AdminTracksModule`     | `admin-tracks/`        | CRUD for uploaded tracks (multipart audio+cover → R2) + bulk operations          | `POST/GET/PATCH/DELETE /api/admin/tracks` (admin-guarded). Bulk: `POST /api/admin/tracks/bulk-delete` and `PATCH /api/admin/tracks/bulk-genre` (cap 200 ids/request, return `{ deleted/updated, notFound }` partition; bulk-delete is best-effort against R2). Bulk routes are declared before `:id` parameterized routes so Nest's declaration-order matching doesn't swallow them.                                                                                       |
...
| `StorageModule`         | `storage/`             | Cloudflare R2 upload/delete via S3-compatible SDK                                | (internal)                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- `admin-tracks/admin-tracks.service.spec.ts` — bulk delete + bulk set-genre partition logic, R2-failure tolerance, dedup, and the empty-input fast path. Uses an in-memory `prisma.track` stub modelling `findMany` / `deleteMany` / `updateMany`.
