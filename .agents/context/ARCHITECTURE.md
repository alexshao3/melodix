...
packages/
  shared/       TypeScript domain types + API_ROUTES + helpers + LRC parser
  ui/           Reusable React components (cards, buttons, motion primitives)
services/
  aeneas-aligner/   Python sidecar (FastAPI + Aeneas + eSpeak + ffmpeg) for
                    forced-alignment of plain-text lyrics → LRC (ADR-0030).
...
| `AdminTracksModule`     | `admin-tracks/`        | CRUD for uploaded tracks (multipart audio+cover → S3-compatible storage) + bulk operations + lyrics auto-sync. `LyricsAlignerService` is an HTTP client for the Aeneas sidecar (ADR-0030) — translates non-2xx → `BadGateway`, abort → `RequestTimeout`. The track update path preserves `syncedLyrics` when `lyrics` is re-sent unchanged, and only invalidates the LRC when the lyrics text actually changes.                                                                                                                                                                                                | `POST/GET/PATCH/DELETE /api/admin/tracks` (admin-guarded). Bulk: `POST /api/admin/tracks/bulk-delete` and `PATCH /api/admin/tracks/bulk-genre` (cap 200 ids/request, return `{ deleted/updated, notFound }` partition; bulk-delete is best-effort against object storage). Lyrics: `POST /api/admin/tracks/:id/auto-sync-lyrics` calls the Aeneas sidecar (5-min timeout) and persists the LRC + `lyricsAlignedAt`. Bulk routes are declared before `:id` parameterized routes so Nest's declaration-order matching doesn't swallow them. |
...
- [`lrc.ts`](../../packages/shared/src/lrc.ts) — `parseLrc(raw)` and
  `findActiveLine(lines, currentTimeS)` (binary search, O(log n) per
  `timeupdate` tick). Used by both `web/LyricsDrawer` and
  `miniapp/LyricsSheet` to render karaoke-style highlighted lyrics.
