...
## ADR-0027 · Self-hosted fonts via `@fontsource-variable/*` instead of `next/font/google`

**Date:** 2026-04-27
**Status:** accepted
**Context:** Operator's `docker compose --build` was failing every time inside their self-hosted environment (`Failed to fetch \`Inter\` from Google Fonts… ETIMEDOUT… Client network socket disconnected before secure TLS connection was established`). The Next.js builder was unable to reach `fonts.googleapis.com` from the build VM (Vietnam ISP / Cloudflare Tunnel host), so the three Next apps (`web`/`miniapp`/`admin`) never got past `Creating an optimized production build…`. CI on GitHub-hosted runners works fine because GitHub's egress isn't restricted, masking the issue from us. Self-hosted builds need to be hermetic.

<!-- prettier-ignore -->
**Decision:** Replace `next/font/google` (`Inter`, `Space_Grotesk`) with the `@fontsource-variable/inter` + `@fontsource-variable/space-grotesk` npm packages in all three Next apps. The fonts ship inside the package (woff2 in `node_modules/.pnpm/@fontsource-variable+*/files/`), so `next build` no longer makes any outbound HTTP request for fonts. Each `app/layout.tsx` imports the package's `index.css` once and sets the existing `--font-sans` / `--font-display` CSS variables via inline `style={fontVars}` on `<html>` so the Tailwind `fontFamily` mapping (`var(--font-sans)` → `font-sans`, `var(--font-display)` → `font-display`) keeps working unchanged. Family names are `'Inter Variable'` and `'Space Grotesk Variable'` (matching the Fontsource convention). The `@fontsource-variable/*` packages were chosen over `next/font/local` because (a) no font binary lands in the repo, (b) Fontsource ships proper unicode-range subsets and `font-display: swap` out of the box.

<!-- prettier-ignore -->
**Consequences:** `docker compose --build` now succeeds on networks that block Google Fonts (firewalls, restrictive ISPs, air-gapped boxes). Trade-off: we lose `next/font`'s automatic preload-link generation and per-build CSS-variable hashing (`__variable_abc123`); in practice the bundled CSS still preloads the right woff2 via `<link rel="stylesheet">` because Next inlines the imported CSS. Bundle size is roughly equivalent — the woff2 files are the same Google would have inlined. If we ever add a third typeface, prefer `@fontsource-variable/*` for consistency.

## ADR-0028 · Use Prisma migrations (not `db push`) and verify them in CI

**Date:** 2026-04-27
**Status:** accepted
**Context:** The production `apps/api/Dockerfile` runs `npx prisma migrate deploy` at container start, but `apps/api/prisma/migrations/` was never committed. The repo had been managed with ad-hoc `prisma db push` against developer / preview databases, which silently kept dev DBs in sync but produced **no migration history**. As a result, every fresh `docker compose up` against a new Postgres (Neon free tier, self-host VM, etc.) booted the API against an empty database and crashed at first query: `PrismaClientKnownRequestError code P2021 — The table public.music_sources does not exist in the current database`. The CI workflow `.github/workflows/ci.yml` only ran typecheck / lint / build, so it never executed `migrate deploy` and never detected the missing migrations. The Playwright e2e workflow uses `prisma db push --accept-data-loss` against a fresh DB on each run, so it also can't detect drift between `schema.prisma` and the migration history.
**Decision:** Adopt Prisma migrations as the authoritative way to evolve the database, replacing `db push` for production.

1. Generate the baseline migration once (`prisma migrate dev --name init` against a clean Postgres) covering all 12 current models (`User`, `Follow`, `PlayHistory`, `Artist`, `Album`, `Track`, `Playlist`, `PlaylistTrack`, `Like`, `AdminUser`, `MusicSource`, `StorageObject`) and check the resulting `prisma/migrations/20260427094947_init/migration.sql` + `migration_lock.toml` into the repo.
2. Keep the existing Dockerfile `CMD` (`prisma migrate deploy && node dist/…/main.js`); now that migrations exist, this actually creates the schema on first boot.
3. Add a new `migrations` job in `.github/workflows/ci.yml` that provisions `postgres:16-alpine` as a service container, runs `prisma migrate deploy`, and then runs `prisma migrate diff --from-schema-datasource prisma/schema.prisma --to-schema-datamodel prisma/schema.prisma --exit-code` to guarantee the live DB (built from migrations) is byte-equal to what `schema.prisma` describes. The diff exits non-zero if `schema.prisma` was edited without a corresponding new migration.
4. Going forward, every PR that touches `schema.prisma` must include a fresh `prisma/migrations/<timestamp>_<slug>/migration.sql` produced by `pnpm --filter @melodix/api prisma:migrate -- --name <slug>`. The CI guard makes the omission impossible to merge.

**Consequences:** Fresh deploys (Neon, self-host Docker, local dev) now create the schema correctly on first boot — this fixes the operator-reported `P2021` crash. **Operational note for existing deploys:** a database that already has the tables (e.g. created via an earlier `db push`) will refuse this baseline migration because it will try to `CREATE TABLE` rows that already exist. Such databases must be marked as already migrated with `npx prisma migrate resolve --applied 20260427094947_init` before the next deploy. Fresh databases (the user's Neon DB, which was throwing `P2021`) need no extra step. Dev workflow changes: `prisma db push` is no longer the recommended way to evolve schema — use `pnpm --filter @melodix/api prisma:migrate -- --name <slug>`. The Playwright e2e workflow keeps using `db push` because it always starts from a clean DB and we want it to work even if migrations are mid-rebase.

## ADR-0029 · Same-origin proxy for Postgres-backed audio (Next.js `rewrites`)
